...
def initialize_llm(model_id: str = None):
    ...
    try:
        ...
        return f"✅ LLM Model loaded: {model_id}"
    except Exception as e:
        return f"❌ Failed to load LLM: {str(e)}"
...
def initialize_stt(model_size: str = "base"):
    ...
    try:
        ...
        return f"✅ STT Model loaded: Whisper {model_size}"
    except Exception as e:
...
def initialize_tts(device: str = "cuda"):
    ...
    try:
        ...
        return f"✅ TTS Model loaded on {device.upper()}"
    except Exception as e:
...
def transcribe_audio(audio_file: str) -> str:
    ...
    if stt_model is None:
        return "⚠️ STT model not initialized"
...
def chat_with_pipeline(
    ...
    if audio_input is not None and pipeline_config in ["Voice to Text", "Voice to Voice"]:
        if stt_model is None:
            return history + [[message or "[Audio input]", "⚠️ STT model not initialized"]], None
        
        transcribed_text = transcribe_audio(audio_input)
        if transcribed_text.startswith("❌") or transcribed_text.startswith("⚠️"):
    ...
    if llm_model is None:
        return history + [[user_input or message, "⚠️ LLM model not initialized"]], None
    ...
    try:
        ...
        return history + [[user_input or message, f"❌ Error: {str(e)}"]], None
        ...
        with gr.Row():
            with gr.Column(scale=2):
                ...
                with gr.Accordion("Model Setup", open=False):
                    ...
                    with gr.Row():
                        llm_init_btn = gr.Button("🧠 Initialize LLM", size="sm")
                    ...
                    with gr.Row():
                        ...
                        stt_init_btn = gr.Button("🎤 Initialize STT", size="sm")
                    ...
                    with gr.Row():
                        ...
                        tts_init_btn = gr.Button("🔊 Initialize TTS", size="sm")
        ...
        with gr.Row():
            with gr.Column(scale=3):
                ...
                with gr.Row():
                    audio_input = gr.Audio(
                        label="🎤 Voice Input (for Voice modes)",
                        type="filepath",
                        sources=["upload", "microphone"]
                    )
                    
                    file_upload = gr.File(
                        label="📎 Upload File (TXT, PDF, DOCX, MD)",
                        file_types=[".txt", ".pdf", ".docx", ".doc", ".md"]
                    )
                
                # Audio Output
                audio_output = gr.Audio(
                    label="🔊 AI Voice Response (for Voice modes)",
                    autoplay=True
                )
            ...
            with gr.Column(scale=1):
                ...
                with gr.Accordion("System Info", open=False):
                    device_info = "🖥️ **GPU:** " + ("✅ Available" if torch.cuda.is_available() else "❌ Not Available")
...
if __name__ == "__main__":
    print("=" * 60)
    print("🚀 Starting Zeyta AI Assistant")
    print("=" * 60)
    print()
    ...
    try:
        ...
        print("⚠️  Faster-Whisper not found (needed for voice features)")
    
    try:
        from transformers import pipeline
        print("✅ Transformers available")
    except ImportError:
        print("❌ Transformers not found - LLM features will not work")
    
    try:
        from chatterbox.tts import ChatterboxTTS
        print("✅ ChatterboxTTS available")
    except ImportError:
        print("⚠️  ChatterboxTTS not found (needed for voice output)")
